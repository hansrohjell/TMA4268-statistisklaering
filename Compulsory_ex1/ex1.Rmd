...
###iv)
A person is 10 years older than another person. The probability of dying is linear in terms of age, so we can see the probability of a person dying at an age of 65 and an age of 75 from task i.
...
```{r}
library(caret)
confusionMatrix(table(predict(logReg, type="response") >= 0.5,
                      test$diabetes == 1))
```
